...
export async function GET(req: NextRequest) {
  ...
  if (oauthError) return NextResponse.redirect(new URL(`/health?error=whoop_${oauthError}`, req.url))
  ...
  await syncWhoopToday(db, user.id, toLocalDate(tz), true, 7).catch(() => null)
