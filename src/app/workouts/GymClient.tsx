...
export default function GymClient({ today, initialConfig, initialExercises, initialBodyWeights }: Props) {
  ...
  function saveEx() {
    const { mode, id, name, gymId, dayIds, bodyweight, startWeight, repMin, repMax, step } = exModal
    if (!name.trim() || !gymId || !dayIds.length) return
    if (mode === 'edit' && id) {
      updateEx.mutate({ id, name: name.trim(), gym_id: gymId, day_ids: dayIds, bodyweight, start_weight: startWeight, rep_min: repMin, rep_max: repMax, step })
    } else {
      createEx.mutate(
        { name: name.trim(), gym_id: gymId, day_ids: dayIds, bodyweight, start_weight: startWeight, rep_min: repMin, rep_max: repMax, step, order_index: exercises.length },
              ...
              <div className="space-y-2">
                <div className="flex gap-2">
                  <input
                    ...
                    step="0.1"
                    value={bwInput}
                    onFocus={e => e.target.select()} onChange={e => setBwInput(e.target.value)}
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div className="flex-1 text-center">
                      <input
                        ...
                        onFocus={e => e.target.select()} onChange={e => { const v = parseFloat(e.target.value); setWeightInput(isNaN(v) ? 0 : v) }}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="number" inputMode="decimal" step="2.5" placeholder="0" value={exModal.startWeight === 0 ? '' : exModal.startWeight}
                      onFocus={e => e.target.select()} onChange={e => { const v = parseFloat(e.target.value); setExModal(m => ({ ...m, startWeight: isNaN(v) ? 0 : v })) }}
          ...
          <div className="w-full rounded-t-3xl bg-[#111] border-t border-white/10 px-5 pt-5 pb-10 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <input
                  type="number" inputMode="numeric" min="1" max="20" value={settingsUpgradeAt}
