...
interface Props {
  today: string
  initialTodayLog: DebloatLog | null
  initialHistory: DebloatLog[]
}
...
export default function DebloatClient({ today, initialTodayLog, initialHistory }: Props) {
  const qc = useQueryClient()

  if (!qc.getQueryData(['debloat-log', today])) {
    qc.setQueryData(['debloat-log', today], initialTodayLog)
  }
  if (!qc.getQueryData(['debloat-history'])) {
    qc.setQueryData(['debloat-history'], initialHistory)
  }
  ...
  // Local state mirrors server state for instant UI response
  const [localBloat, setLocalBloat] = useState<number | null>(initialTodayLog?.bloat_level ?? null)
  const [localChecklist, setLocalChecklist] = useState<Record<string, boolean>>(
    initialTodayLog?.checklist ?? {}
  )
  ...
  return (
    <main className="min-h-screen bg-black text-white pb-28 pt-6">
      <div className="px-5 space-y-5">

        {/* Header */}
        <div className="flex items-center justify-between">
          <h1 className="text-3xl font-bold tracking-tight">Debloat</h1>
        ...
        <section className="rounded-2xl bg-zinc-900 p-5">
          ...
          <p className="text-xs text-zinc-600 mb-4">
            Take a selfie and get feedback on facial puffiness and what to do
          </p>
          ...
          {previewUrl && (
            <div className="mb-4 rounded-xl overflow-hidden">
              ...
              <img src={previewUrl} alt="Food preview" className="w-full max-h-48 object-cover" />
            </div>
          )}
                ...
                {openGuide === i && (
                  <div className="px-5 pb-5 flex flex-col gap-3 border-t border-zinc-800">
                    {section.items.map((item, j) => (
                      <p key={j} className="text-sm text-zinc-400 leading-relaxed pt-3">
                        {item}
                      </p>
                    ))}
                  </div>
