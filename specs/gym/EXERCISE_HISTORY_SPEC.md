...
- **Points** (locked in the lab): **solid** accent dots (thin bg rim for definition — **never hollow**; small hollow dots read as holes/arrows) at each session **for short ranges only**. **Beyond ~16 visible sessions → no static dots, line only** — pure Apple-stocks: you drag along the line and the crosshair + a single active dot appear at your finger, nothing to clutter or aim at. The scrubbed/active point always renders (and the latest point shows by default). For very long histories, downsample/bucket (weekly→monthly) before plotting; line-only + scrub already covers the volume.
