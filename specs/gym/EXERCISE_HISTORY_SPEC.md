...
- **Points** (locked in the lab): **hollow ringed dots** (bg fill + accent stroke — the reference look Luka likes) at each session **for short ranges only (≤16 sessions: W/M/3M)**. **Beyond ~16 → no static dots, line only** — pure Apple-stocks: drag along the line and the crosshair + a single active dot appear at your finger. Dots only ever render at ≤16 points so they're always large enough that the ring reads clean (the holes/arrows problem only happened when dots shrank tiny on long ranges — those no longer render dots at all). The scrubbed/active point is a **solid** filled dot + halo and always renders (latest shows by default). For very long histories, downsample/bucket (weekly→monthly); line-only + scrub already covers the volume.
...
- **Particle burst — LOCKED default = "Sparks + shockwave ring, NO flash"** (chosen in the lab): one crisp radial burst of ~22 amber/white shards fired from the star (angle/distance/rotation from the element **index**, **not** `Math.random()` — deterministic; SSR/workflow-safe), springing out then fading, plus **one expanding shockwave ring**. **No screen flash** — Luka found it too much. Earned + clean without the busy multi-pop of Fireworks, which matters because early in a lift you PR most sessions. (Lab keeps `fireworks`/`confetti`/`minimal` + a flash toggle for reference; **Fireworks is a good optional escalation reserved for a true all-time-milestone PR** — a nice-to-have, not v1.)
